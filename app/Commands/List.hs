...
getFieldSizes refs = do
  ...
  -- Author field.
  let getMaxAuthorLength :: Reference -> Int
      getMaxAuthorLength =
...
getShortestJournalName =
  replaceAcronyms . T.strip . T.filter ((||) <$> isAlphaNum <*> isSpace)
    . view
      (work . journalShort)
  where
