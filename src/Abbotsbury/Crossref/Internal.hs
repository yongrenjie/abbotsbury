...
getCrossrefJson manager email doi' = do
  ...
  request <-
    NHC.setQueryString [("mailto", Just emailBS)]
...
parseArticle useInternalAbbrevs messageObj = do
  ...
  _articleTitle <-
    processArticleTitle
      <$> safeHeadWithDef "" (messageObj .: "title")
  -- Authors
  ...
  _articleJournalShort <-
    case (useInternalAbbrevs, getJournalAbbrev _articleJournalLong) of
      ...
      _ ->
        safeHead "" (messageObj .: "short-container-title")
  ...
  _articleYear <-
    safeHead "year not found in date-parts" $
...
-- | Modify a parser to return the first item of a list instead of the entire
-- list. If the list is empty, fail with the given error message.
safeHead ::
...
-- | Like 'safeHead', but instead of failing, uses a default value to return if
-- the list is empty.
safeHeadWithDef :: a -> DAT.Parser [a] -> DAT.Parser a
safeHeadWithDef def listParser = do
  list <- listParser
  case list of
    [] -> pure def
    (x : _) -> pure x
...
separateInitials t = T.concat . zipWith f [1 ..] . T.unpack $ t
  where
    ...
    f :: Int -> Char -> Text
    f index c =
      if c == '.' && notElem nextChar [Nothing, Just '-', Just ' ']
        then ". "
        else T.singleton c
      where
